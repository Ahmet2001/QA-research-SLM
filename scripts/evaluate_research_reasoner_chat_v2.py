#!/usr/bin/env python3
"""Chat-template evaluator with JSON extraction for Qwen-style thinking outputs."""
...
def extract_first_json_object(text: str) -> str:
    """Return the first balanced JSON object substring, or original text."""
    start = text.find("{")
    if start < 0:
        return text
    depth = 0
...
def build_prompt(row: Dict[str, Any], tokenizer) -> str:
    ...
    # Add a hard decoding instruction only at eval-time to test controllability.
    if prompt_messages and prompt_messages[-1].get("role") == "user":
        prompt_messages[-1] = dict(prompt_messages[-1])
        prompt_messages[-1]["content"] += "\n\nReturn only one complete JSON object. Do not include prose or <think>."
...
def evaluate(name: str, base_model: str, adapter: Optional[str], rows: List[Dict[str, Any]], max_new_tokens: int) -> Dict[str, Any]:
    ...
    for row in rows:
        ...
        extracted = extract_first_json_object(completion)
        raw_scores = score_output(completion, make_prompt_item(row))
        extracted_scores = score_output(extracted, make_prompt_item(row))
        totals_raw.append(raw_scores["total"])
        totals_extracted.append(extracted_scores["total"])
        results.append({
            "id": row.get("id"),
            "completion": completion,
            "extracted_json_candidate": extracted,
    ...
    return {
        ...
        "adapter": adapter,
        "n": len(rows),
        "mean_total_raw": sum(totals_raw) / max(1, len(totals_raw)),
        "mean_total_extracted": sum(totals_extracted) / max(1, len(totals_extracted)),
        "results": results,
    }
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "out": args.out,
        "summary": [
            {"name": r["name"], "n": r["n"], "raw": r["mean_total_raw"], "extracted": r["mean_total_extracted"]}
            for r in out["runs"]
        ],
