...
def main() -> None:
    ...
    tok = AutoTokenizer.from_pretrained(init_adapter or base_model, trust_remote_code=True, use_fast=True)
    ...
    if init_adapter:
        ...
        print('CONTINUE_FROM_ADAPTER', init_adapter)
    else:
        lc = cfg['lora']
    ...
    kwargs = dict(
        ...
        gradient_accumulation_steps=int(batching.get('gradient_accumulation_steps', 4)),
        num_train_epochs=float(batching.get('num_train_epochs', 1)),
        learning_rate=float(opt.get('learning_rate', 5e-6)),
        ...
        logging_steps=int(logging.get('logging_steps', 5)),
        save_steps=int(logging.get('save_steps', 100)),
        save_total_limit=2,
        bf16=True,
    ...
    if 'validation' in tokenized:
        ...
        kwargs['eval_steps'] = int(logging.get('eval_steps', 20))
    ...
    print('SFT_CONTINUE_OK', output_dir)
