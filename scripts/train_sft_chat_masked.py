...
def main() -> None:
    ...
    model = get_peft_model(model, LoraConfig(
        r=int(lc.get('r', 32)), lora_alpha=int(lc.get('alpha', 64)), lora_dropout=float(lc.get('dropout', 0.05)),
        bias='none', task_type='CAUSAL_LM', target_modules=lc.get('target_modules')
    ))
    ...
    def encode(ex: Dict[str, Any]) -> Dict[str, Any]:
        msgs = ex['messages']
        prompt_msgs = msgs[:-1]
        prompt = chat_text(tokenizer, prompt_msgs, True)
        full = chat_text(tokenizer, msgs, False)
    ...
    tokenized = ds.map(encode, remove_columns=remove_cols)
    ...
    kwargs = dict(
        ...
        gradient_accumulation_steps=int(batching.get('gradient_accumulation_steps', 8)),
        num_train_epochs=float(batching.get('num_train_epochs', 1)),
        ...
        save_steps=int(logging.get('save_steps', 250)),
        save_total_limit=2,
        bf16=True,
    ...
    trainer = Trainer(model=model, args=training_args, train_dataset=tokenized['train'], eval_dataset=tokenized.get('validation'), data_collator=Collator(tokenizer))
    trainer.train()
    ...
    print('SFT_CHAT_MASKED_OK', output_dir)
