...
def prompt_item(row: Dict[str, Any]) -> Dict[str, Any]:
    sources = row.get('sources', [])
    target = row.get('target', {})
    return {
        'sources': [{'source_id': s.get('source_id'), 'status': s.get('status', 'unknown'), 'text': s.get('text', '')} for s in sources],
        'verifier_hints': {
...
def load_model(base_model: str, adapter: str | None):
    ...
    model = AutoModelForCausalLM.from_pretrained(base_model, quantization_config=bnb, device_map='auto', trust_remote_code=True, torch_dtype=torch.bfloat16, attn_implementation='sdpa')
    if adapter:
...
def gen(model, tok, row: Dict[str, Any], max_new_tokens: int) -> str:
    import torch
    msgs = [m for m in row.get('messages', []) if m.get('role') != 'assistant']
...
def evaluate(name: str, base: str, adapter: str | None, rows: List[Dict[str, Any]], max_new_tokens: int) -> Dict[str, Any]:
    model, tok = load_model(base, adapter)
    results, totals = [], []
    for row in rows:
        completion = gen(model, tok, row, max_new_tokens)
        scores = score_output(completion, prompt_item(row))
...
def main() -> None:
    ...
    print(json.dumps({'ok': True, 'summary': [{'name': r['name'], 'n': r['n'], 'mean_total': r['mean_total']} for r in out['runs']], 'out': args.out}, ensure_ascii=False, indent=2))
