#!/bin/bash
#SBATCH --job-name=rr_eval_safe
#SBATCH --output=/arf/home/egitimg16u5/chatgpt_workspaces/research_reasoner_v0/logs/rr_eval_safe_%j.out
...
python3 scripts/evaluate_research_reasoner_flat_safe.py \
  ...
  --out eval_outputs/research_reasoner_real_compact_flat_eval_safe.json \
  --compare-base \
  --max-new-tokens 1024 \
  --limit 8
